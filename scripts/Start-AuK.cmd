...
powershell.exe -NoProfile -Command "try { $r=Invoke-RestMethod -Uri 'http://127.0.0.1:7860/api/v1/health' -TimeoutSec 2; if ($r.status -eq 'ok' -and $r.ui_enabled -eq $true) { exit 0 } } catch {}; exit 1"
if not errorlevel 1 (
